...
class GameGrid:
    ...
    def clear_full_lines(self):
        # List to store the indices of full lines
        full_lines = []
        # Check each row in the grid for full lines
        for row in range(self.grid_height):
            # If all cells in a row are occupied
            if all(self.tile_matrix[row]):
                full_lines.append(row)  # Add the index of the full line to the list

        # If there are full lines to clear
        if full_lines:
            # Clear the full lines by shifting the rows above down
            for row in reversed(full_lines):
                # Shift all rows above the full line down by one row
                for r in range(row, self.grid_height - 1):
                    self.tile_matrix[r] = np.copy(self.tile_matrix[r + 1])
    ...
    def update_grid(self, tiles_to_lock, blc_position):
        ...
        self.merge_tiles()  # Merge the tiles after locking
        return self.game_over

    def merge_tiles(self):
        for col in range(self.grid_width):
            for row in range(self.grid_height - 1, 0, -1):
                if self.tile_matrix[row][col] is not None and self.tile_matrix[row][col] == self.tile_matrix[row - 1][col]:
